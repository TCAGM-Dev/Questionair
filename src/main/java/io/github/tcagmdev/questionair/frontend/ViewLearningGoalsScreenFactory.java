...
public abstract class ViewLearningGoalsScreenFactory {
	public static StateNode<String> createNode(StateMachine<String> stateMachine, StateNode<String> parent, CLIFrontend frontend) {
		...
		StateNode<String> node = stateMachine.addNode(_ -> {
			frontend.out.println("Learning goals:");

			idMap.clear();
			int currentIndex = 1;
			for (Map.Entry<Integer, LearningGoal> entry : frontend.getApp().DATA.getGoals().entrySet()) {
				idMap.put(String.valueOf(currentIndex), entry.getKey());
				frontend.out.printf("%d. %s%n", currentIndex, entry.getValue().getDescription());
				currentIndex++;
			}

			if (frontend.getApp().DATA.getGoals().isEmpty()) frontend.out.println("None");
		...
		node.addConnection(idMap::containsKey, input -> LearningGoalInfoScreenFactory.getOrCreateNode(stateMachine, node, frontend, frontend.getApp().DATA.getGoal(idMap.get(input))));
